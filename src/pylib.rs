...
fn json_value_to_python(py: Python, value: &serde_json::Value) -> PyResult<PyObject> {
    match value {
        ...
        serde_json::Value::Bool(b) => Ok(b.into_pyobject(py)),
        serde_json::Value::Number(n) => {
            if let Some(i) = n.as_i64() {
                ...
                Ok(n.to_string().into_pyobject(py))
            }
        }
        serde_json::Value::String(s) => Ok(s.into_pyobject(py)),
        serde_json::Value::Array(arr) => {
            let py_list = PyList::empty_bound(py);
            for item in arr {
                py_list.append(json_value_to_python(py, item)?)?;
            }
            Ok(py_list.into_pyobject(py))
        }
