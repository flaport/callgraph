...
pub fn analyze_python_file(
    ...
    match parse_module(&content) {
        ...
        Err(parse_error) => {
            ...
            debug!("Full module parsing failed, attempting partial parsing for {}: {:?}", file_path.display(), parse_error);
...
fn parse_python_partially(content: &str, file_path: &Path) -> (Vec<ruff_python_ast::Stmt>, Vec<String>) {
    ...
    for (block_content, line_start) in blocks {
        match try_parse_block(&block_content) {
            Ok(stmts) => {
                debug!("Successfully parsed block starting at line {} in {}", line_start, file_path.display());
                statements.extend(stmts);
            }
            Err(e) => {
                let error_msg = format!("Parse error at line {}: {:?}", line_start, e);
                debug!("Failed to parse block starting at line {} in {}: {:?}", line_start, file_path.display(), e);
